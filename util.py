...
def make_genome_dir(request_id, form_data):
    ...
    os.chmod(os.path.join(settings.QUERY_FOLDER, '{0}/genomes'.format(str(request_id))), PEM_BITS)
    for key, value in form_data.iteritems():
        if 'orgList' in key and value is not unicode(''):
            ...
            os.chmod(os.path.join(settings.QUERY_FOLDER, '{0}/genomes/{1}'.format(str(request_id), genome_dir)), PEM_BITS)
...
def make_query_file(request_id, form_data):
    ...
    os.chmod(os.path.join(settings.QUERY_FOLDER, '{0}/phylo_tree.txt'.format(str(request_id))), PEM_BITS)
    for f in files:
...
def make_operon_filter(request_id, form_data):
    ...
    os.chmod(os.path.join(settings.QUERY_FOLDER, '{0}/filter_file'.format(str(request_id))), PEM_BITS)
    operon_list = []
    for key, value in form_data.iteritems():
...
def make_operon_dir(request_id, form_data):
    ...
    os.chmod(os.path.join(settings.QUERY_FOLDER, '{0}/operons'.format(str(request_id))), PEM_BITS)
