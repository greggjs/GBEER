...
def return_tree_order_list(newick_tree_file, common_to_accession_dict, request_id=None):
    result = []
    ...
    for clade in tree.find_clades():
        if clade.name != None:
            ...
            clade = common
    print '\n'.join(result)
    outfile_for_asma = './queries/{0}/asma_outlist.txt'.format(request_id) if request_id else './asma_outlist.txt'
    handle = open(outfile_for_asma, 'w')
    handle.write('\n'.join(result))
    handle.close()

    phylo_order_new = './queries/{0}/phylo_order_new.txt'.format(request_id) if request_id else './phylo_order_new.txt'
...
def get_newick_tree(infolder, outfile, filter_file, request_id):
    start = time.time()
    ...
    #return_tree_order_list(tree_file, common_to_accession_dict)
    return_tree_order_list(outfile, common_to_accession_dict, request_id=request_id)
