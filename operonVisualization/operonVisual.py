...
def changeStrandDirection(eleTup):
    newEleTup = (eleTup[0],eleTup[2],eleTup[1],'+1')
    return newEleTup

def changeStrandDirectionOption(eleTup,direction):
    newEleTup = (eleTup[0],eleTup[2],eleTup[1],direction)
    return newEleTup
...
def drawGenomeDiag(result_Dict,phylo_order,filename,OutputDirectory):
    ...
    idToColorDict_reportLab,idToColorDict_matplotlib=geneToColor(set(geneList))
...
def reading_MappingFile(mappingFile):
    ...
    accession = []
    organism = []
    for Accession_Organism in reader:
        print Accession_Organism
        accession.append(Accession_Organism[0])
...
def create_operon_images(operon_data_dir, mapping_file, newick_tree, event_dict, output_dir, request_id):
    all_colors = list(reportlab.lib.colors.getAllNamedColors().items())
    condition = chk_output_directory_path(output_dir, request_id)
    if condition:
       outputsession = output_dir + "_" + str(request_id)
       OutputGenomeDiagDirectory = makeSubfolder(outputsession,"genome-diagrams", request_id)
       OutputCSVDirectory = makeSubfolder(outputsession,"operon-event-matrices", request_id)
       OutputTreeGDHeatDirectory = makeSubfolder(outputsession, "tree-gd-heat-diagrams", request_id)
       TempDirectory = makeSubfolder(outputsession, "temporary-files", request_id)
       accession_order,organism_order = reading_MappingFile(open(mapping_file, 'r'))
       res = traverseAll(operon_data_dir)
       legendData = {}
       print "*****"
       ...
       pickleToCSV.generateCombined(open(event_dict, 'r'), legendData, accession_order, organism_order, newick_tree,
            OutputCSVDirectory, OutputGenomeDiagDirectory, OutputTreeGDHeatDirectory, TempDirectory)
